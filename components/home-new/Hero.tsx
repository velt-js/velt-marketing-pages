...
import { DEPLOYMENT_BADGES, EU_RESIDENCY_BADGE } from "@/lib/deployment";
...
// Badge artwork, keyed by the label it illustrates. Labels themselves come
// from lib/deployment.ts so the hero never hardcodes the trust badge set; an
// unmapped label falls back to the shield.
const BADGE_ICONS: Record<string, ReactNode> = {
  "Self-hosted": (
    <svg className="hero-badge-icon" viewBox="0 0 16 16" fill="none" aria-hidden="true">
    <path d="M3 2.5h10a1.5 1.5 0 0 1 1.5 1.5v2A1.5 1.5 0 0 1 13 7.5H3A1.5 1.5 0 0 1 1.5 6V4A1.5 1.5 0 0 1 3 2.5Zm0 6h10A1.5 1.5 0 0 1 14.5 10v2a1.5 1.5 0 0 1-1.5 1.5H3A1.5 1.5 0 0 1 1.5 12v-2A1.5 1.5 0 0 1 3 8.5Z" fill="currentColor" />
    </svg>
  ),
  "SOC 2 Type II": (
    <svg className="hero-badge-icon" viewBox="0 0 16 16" fill="none" aria-hidden="true">
...
// DEPLOYMENT_BADGES first, then the EU residency badge, which is kept
// alongside them rather than replaced by them.
const HERO_BADGES: string[] = [...DEPLOYMENT_BADGES, EU_RESIDENCY_BADGE];
          ...
          <div className="hero-badges">
            {HERO_BADGES.map((badge) => (
              <span className="hero-badge" key={badge}>
                {BADGE_ICONS[badge] ?? BADGE_ICONS["SOC 2 Type II"]}
