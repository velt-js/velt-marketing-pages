...
const ASSET_URLS_PATH = resolve(
  PROJECT_ROOT,
  "scripts/integration-asset-urls.json",
);
...
async function fetchAndUpload(url) {
  if (!url) return null;
  if (DRY_RUN)
    return { _type: "image", asset: { _ref: `dry-run-${basename(url)}` } };
...
async function main() {
  ...
  if (ONLY && targets.length === 0) {
    console.error(
      `No entry matching slug "${ONLY}" in integration-content.json`,
    );
    process.exit(1);
  }

  console.log(
    `${DRY_RUN ? "[DRY RUN] " : ""}Seeding ${targets.length} integration(s)...`,
  );
