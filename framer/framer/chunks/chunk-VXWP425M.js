...
// /:https://framerusercontent.com/modules/wFkXxZqfKOPeEQtsYrsa/TWvikt1kG1sKU5G0pMRF/codemirror_state.js
var rangeFrom = [];
var rangeTo = [];
(() => {
  ...
  for (let i2 = 0, n = 0; i2 < numbers.length; i2++) (i2 % 2 ? rangeTo : rangeFrom).push(n = n + numbers[i2]);
})();
...
var ChangeSet = class _ChangeSet extends ChangeDesc {
  ...
  */
  filter(ranges) {
    ...
    return { changes: new _ChangeSet(resultSections, resultInserted), filtered: ChangeDesc.create(filteredSections) };
  }
  /**
...
function resolveTransactionInner(state, spec, docSize) {
  ...
  return { changes: spec.changes instanceof ChangeSet ? spec.changes : ChangeSet.of(spec.changes || [], docSize, state.facet(lineSeparator)), selection: sel && (sel instanceof EditorSelection ? sel : EditorSelection.single(sel.anchor, sel.head)), effects: asArray(spec.effects), annotations, scrollIntoView: !!spec.scrollIntoView };
}
...
var EditorState = class _EditorState {
  ...
  */
  replaceSelection(text) {
    ...
    return this.changeByRange((range) => ({ changes: { from: range.from, to: range.to, insert: text }, range: EditorSelection.cursor(range.from + text.length) }));
  }
  /**
  ...
  */
  static fromJSON(json, config = {}, fields) {
    ...
    return _EditorState.create({ doc: json.doc, selection: EditorSelection.fromJSON(json.selection), extensions: config.extensions ? fieldInit.concat([config.extensions]) : fieldInit });
  }
  /**
  ...
  */
  phrase(phrase, ...insert2) {
    ...
    if (insert2.length) phrase = phrase.replace(/\$(\$|\d*)/g, (m, i2) => {
      if (i2 == "$") return "$";
      let n = +(i2 || 1);
  ...
  */
  static join(sets) {
    ...
    for (let i2 = sets.length - 2; i2 >= 0; i2--) {
      for (let layer = sets[i2]; layer != _RangeSet.empty; layer = layer.nextLayer) result = new _RangeSet(layer.chunkPos, layer.chunk, result, Math.max(layer.maxPoint, result.maxPoint));
    }
    return result;
  }
...
// /:https://framerusercontent.com/modules/wKCR2JyUW8wZdYweJrou/b3PAgW7qmQJC7rQUi9po/codemirror_view.js
...
var top = typeof globalThis != "undefined" ? globalThis : typeof __unframerWindow != "undefined" ? __unframerWindow : {};
...
function windowRect(win) {
  ...
  if (vp) return { left: 0, right: vp.width, top: 0, bottom: vp.height };
...
function scrollRectIntoView(dom, rect, side, x, y, xMargin, yMargin, ltr) {
  let doc2 = dom.ownerDocument, win = doc2.defaultView || __unframerWindow;
  for (let cur = dom, stop = false; cur && !stop; ) {
    if (cur.nodeType == 1) {
      ...
      if (top22) {
        ...
        bounding = { left: rect2.left, right: rect2.left + cur.clientWidth * scaleX, top: rect2.top, bottom: rect2.top + cur.clientHeight * scaleY };
      }
      let moveX = 0, moveY = 0;
      if (y == "nearest") {
      ...
      if (top22) break;
      if (rect.top < bounding.top || rect.bottom > bounding.bottom || rect.left < bounding.left || rect.right > bounding.right) rect = { left: Math.max(rect.left, bounding.left), right: Math.min(rect.right, bounding.right), top: Math.max(rect.top, bounding.top), bottom: Math.min(rect.bottom, bounding.bottom) };
      cur = cur.assignedSlot || cur.parentNode;
...
function logException(state, exception, context) {
  ...
  else if (__unframerWindow.onerror) __unframerWindow.onerror(String(exception), context, void 0, void 0, exception);
...
function getIsolatedRanges(view, line) {
  ...
  let result = [];
  RangeSet.spans(sets, line.from, line.to, { point() {
...
var DocView = class extends ContentView {
  ...
  coordsForChar(pos) {
    ...
    while (child.children.length) {
      ...
      for (; ; i22++) {
        if (i22 == child.children.length) return null;
        if ((child = child.children[i22]).length) break;
      }
      off = childOff;
    }
    ...
    for (let i22 = 0; i22 < rects.length; i22++) {
      let rect = rects[i22];
      if (i22 == rects.length - 1 || rect.top < rect.bottom && rect.left < rect.right) return rect;
    }
    return null;
  }
  ...
  computeBlockGapDeco() {
    ...
    for (let pos = 0, i2 = 0; ; i2++) {
      ...
      if (end > pos) {
        ...
        deco.push(Decoration.replace({ widget: new BlockGapWidget(height), block: true, inclusive: true, isBlockGap: true }).range(pos, end));
      }
      if (!next) break;
      pos = next.to + 1;
    }
  ...
  updateDeco() {
    ...
    let dynamicOuter = false, outerDeco = this.view.state.facet(outerDecorations).map((d, i22) => {
  ...
  scrollIntoView(target) {
    ...
    if (!rect) return;
    if (!range.empty && (other = this.coordsAt(range.anchor, range.anchor > range.head ? -1 : 1))) rect = { left: Math.min(rect.left, other.left), top: Math.min(rect.top, other.top), right: Math.max(rect.right, other.right), bottom: Math.max(rect.bottom, other.bottom) };
    let margins = getScrollMargins(this.view);
    let targetRect = { left: rect.left - margins.left, top: rect.top - margins.top, right: rect.right + margins.right, bottom: rect.bottom + margins.bottom };
...
function findCompositionRange(view, changes, headPos) {
  ...
  for (let parent = textNode.parentNode; ; parent = parent.parentNode) {
    ...
    else if (parentView instanceof LineView || parent.nodeName == "DIV" && parent.parentNode == view.contentDOM) return { range, text: textNode, marks, line: parent };
    else if (parent != view.contentDOM) marks.push({ node: parent, deco: new MarkDecoration({ inclusive: true, attributes: getAttrs(parent), tagName: parent.tagName.toLowerCase() }) });
    else return null;
  }
}
...
function moveToLineBoundary(view, start, forward, includeWrap) {
  ...
  if (coords) {
    ...
    let pos = view.posAtCoords({ x: forward == (direction == Direction.LTR) ? editorRect.right - 1 : editorRect.left + 1, y: (coords.top + coords.bottom) / 2 });
...
function applyDOMChange(view, domChange) {
  ...
  if (domChange.bounds) {
    ...
    if (diff) {
      ...
      change = { from: from + diff.from, to: from + diff.toA, insert: Text.of(domChange.text.slice(diff.from, diff.toB).split(LineBreakPlaceholder)) };
    }
  ...
  if (!change && domChange.typeOver && !sel.empty && newSel && newSel.main.empty) {
    ...
    change = { from: sel.from, to: sel.to, insert: view.state.doc.slice(sel.from, change.from).append(change.insert).append(view.state.doc.slice(change.to, sel.to)) };
  ...
  if (change.from >= sel.from && change.to <= sel.to && change.to - change.from >= (sel.to - sel.from) / 3 && (!newSel || newSel.main.empty && newSel.main.from == change.from + change.insert.length) && view.inputState.composing < 0) {
    ...
    if (startState.selection.ranges.length > 1 && view.inputState.composing >= 0 && change.to <= sel.to && change.to >= sel.to - 10) {
      ...
      tr = startState.changeByRange((range) => {
        ...
        // ignored.
        range.to >= compositionRange.from && range.from <= compositionRange.to) return { range };
        let rangeChanges = startState.changes({ from, to, insert: change.insert }), selOff = range.to - sel.to;
        return { changes: rangeChanges, range: !mainSel ? range.map(rangeChanges) : EditorSelection.range(Math.max(0, mainSel.anchor + selOff), Math.max(0, mainSel.head + selOff)) };
      });
    } else {
...
var PendingKeys = [{ key: "Backspace", keyCode: 8, inputType: "deleteContentBackward" }, { key: "Enter", keyCode: 13, inputType: "insertParagraph" }, { key: "Enter", keyCode: 13, inputType: "insertLineBreak" }, { key: "Delete", keyCode: 46, inputType: "deleteContentForward" }];
...
var MouseSelection = class {
  ...
  move(event) {
    ...
    let sx = 0, sy = 0;
    let left = 0, top22 = 0, right = this.view.win.innerWidth, bottom = this.view.win.innerHeight;
    if (this.scrollParents.x) ({ left, right } = this.scrollParents.x.getBoundingClientRect());
    if (this.scrollParents.y) ({ top: top22, bottom } = this.scrollParents.y.getBoundingClientRect());
    let margins = getScrollMargins(this.view);
    ...
    if (event.clientY - margins.top <= top22 + dragScrollMargin) sy = -dragScrollSpeed(top22 - event.clientY);
  ...
  select(event) {
    ...
    if (this.mustSelect || !selection.eq(view.state.selection, this.dragging === false)) this.view.dispatch({ selection, userEvent: "select.pointer" });
...
function dropText(view, event, text, direct) {
  ...
  view.focus();
  view.dispatch({ changes, selection: { anchor: changes.mapPos(dropPos, -1), head: changes.mapPos(dropPos, 1) }, userEvent: del ? "move.drop" : "input.drop" });
  view.inputState.draggedContent = null;
}
...
handlers.beforeinput = (view, event) => {
  ...
  if (browser.chrome && browser.android && (pending = PendingKeys.find((key) => key.inputType == event.inputType))) {
    ...
    if (pending.key == "Backspace" || pending.key == "Delete") {
      let startViewHeight = ((_b = __unframerWindow.visualViewport) === null || _b === void 0 ? void 0 : _b.height) || 0;
      setTimeout(() => {
        var _a22;
        if ((((_a22 = __unframerWindow.visualViewport) === null || _a22 === void 0 ? void 0 : _a22.height) || 0) > startViewHeight + 10 && view.hasFocus) {
...
var HeightMapGap = class _HeightMapGap extends HeightMap {
  ...
  blockAt(height, oracle, top22, offset) {
    ...
    if (oracle.lineWrapping) {
      let guess = offset + (height < oracle.lineHeight ? 0 : Math.round(Math.max(0, Math.min(1, (height - top22) / this.height)) * this.length));
...
var HeightMapBranch = class extends HeightMap {
  ...
  blockAt(height, oracle, top22, offset) {
    ...
    return height < mid ? this.left.blockAt(height, oracle, top22, offset) : this.right.blockAt(height, oracle, mid, offset + this.left.length + this.break);
  }
  lineAt(value, type, oracle, top22, offset) {
    let rightTop = top22 + this.left.height, rightOffset = offset + this.left.length + this.break;
    let left = type == QueryType.ByHeight ? value < rightTop : value < rightOffset;
    let base2 = left ? this.left.lineAt(value, type, oracle, top22, offset) : this.right.lineAt(value, type, oracle, rightTop, rightOffset);
...
function visiblePixelRange(dom, paddingTop) {
  ...
  for (let parent = dom.parentNode; parent && parent != doc2.body; ) {
    if (parent.nodeType == 1) {
      let elt = parent;
      let style = __unframerWindow.getComputedStyle(elt);
  ...
  return { left: left - rect.left, right: Math.max(left, right) - rect.left, top: top22 - (rect.top + paddingTop), bottom: Math.max(top22, bottom) - (rect.top + paddingTop) };
}
function inWindow(elt) {
  let rect = elt.getBoundingClientRect(), win = elt.ownerDocument.defaultView || __unframerWindow;
...
var ViewState = class {
  ...
  measure(view) {
    let dom = view.contentDOM, style = __unframerWindow.getComputedStyle(dom);
  ...
  viewportIsAppropriate({ from, to }, bias = 0) {
    ...
    let { visibleTop, visibleBottom } = this;
    return (from == 0 || top22 <= visibleTop - Math.max(10, Math.min(-bias, 250))) && (to == this.state.doc.length || bottom >= visibleBottom + Math.max(10, Math.min(bias, 250))) && top22 > visibleTop - 2 * 1e3 && bottom < visibleBottom + 2 * 1e3;
  }
  ...
  computeVisibleRanges(changes) {
    ...
    let ranges = [];
    RangeSet.spans(deco, this.viewport.from, this.viewport.to, { span(from, to) {
      ranges.push({ from, to });
    }, point() {
    } }, 20);
    let changed = 0;
  ...
  constructor(state) {
    this.state = state;
    this.pixelViewport = { left: 0, right: __unframerWindow.innerWidth, top: 0, bottom: 0 };
    this.inView = true;
    this.paddingTop = 0;
...
function lineStructure(from, to, stateDeco) {
  ...
  RangeSet.spans(stateDeco, from, to, { span() {
  }, point(from2, to2) {
    if (from2 > pos) {
...
var DOMObserver = class {
  ...
  readMutation(rec) {
    ...
    if (rec.type == "childList") {
      ...
      return { from: childBefore ? cView.posAfter(childBefore) : cView.posAtStart, to: childAfter ? cView.posBefore(childAfter) : cView.posAtEnd, typeOver: false };
  ...
  constructor(view) {
    ...
    if (__unframerWindow.EditContext && view.constructor.EDIT_CONTEXT !== false && // Chrome <126 doesn't support inverted selections in edit context (#1392)
    ...
    if (__unframerWindow.matchMedia) this.printQuery = __unframerWindow.matchMedia("print");
...
var EditContextManager = class {
  ...
  constructor(view) {
    ...
    let context = this.editContext = new __unframerWindow.EditContext({ text: view.state.doc.sliceString(this.from, this.to), selectionStart: this.toContextPos(Math.max(this.from, Math.min(this.to, view.state.selection.main.anchor))), selectionEnd: this.toContextPos(view.state.selection.main.head) });
    this.handlers.textupdate = (e) => {
      let main = view.state.selection.main, { anchor, head } = main;
      let from = this.toEditorPos(e.updateRangeStart), to = this.toEditorPos(e.updateRangeEnd);
      if (view.inputState.composing >= 0 && !this.composing) this.composing = { contextBase: e.updateRangeStart, editorBase: from, drifted: false };
      ...
      if ((browser.mac || browser.android) && change.from == head - 1 && /^\. ?$/.test(e.text) && view.contentDOM.getAttribute("autocorrect") == "off") change = { from, to, insert: Text.of([e.text.replace(".", " ")]) };
...
var EditorView = class _EditorView {
  ...
  */
  get win() {
    return this.dom.ownerDocument.defaultView || __unframerWindow;
  }
  dispatch(...input) {
  ...
  measure(flush = true) {
    ...
    try {
      for (let i2 = 0; ; i2++) {
        ...
        for (let i22 = 0; i22 < measuring.length; i22++) if (measured[i22] != BadMeasure) {
          try {
            let m = measuring[i22];
  ...
  */
  ...
  updateAttrs() {
    ...
    let contentAttrs = { spellcheck: "false", autocorrect: "off", autocapitalize: "off", writingsuggestions: "false", translate: "no", contenteditable: !this.state.facet(editable) ? "false" : "true", class: "cm-content", style: `${browser.tabSize}: ${this.state.tabSize}`, role: "textbox", "aria-multiline": "true" };
  ...
  */
  setRoot(root) {
    if (this._root != root) {
      this._root = root;
      this.observer.setWindow((root.nodeType == 9 ? root : root.ownerDocument).defaultView || __unframerWindow);
      this.mountStyles();
    }
  }
  /**
...
function buildKeymap(bindings, platform = currentPlatform) {
  ...
  let add = (scope, key, command, preventDefault, stopPropagation) => {
    ...
    for (let i2 = 1; i2 < parts.length; i2++) {
      ...
      if (!scopeObj[prefix]) scopeObj[prefix] = { preventDefault: true, stopPropagation: false, run: [(view) => {
    ...
    let binding = scopeObj[full] || (scopeObj[full] = { preventDefault: false, stopPropagation: false, run: ((_b = (_a2 = scopeObj._any) === null || _a2 === void 0 ? void 0 : _a2.run) === null || _b === void 0 ? void 0 : _b.slice()) || [] });
  ...
  for (let b of bindings) {
    ...
    for (let scope of scopes) {
      ...
      if (b.shift) add(scope, "Shift-" + name2, b.shift, b.preventDefault, b.stopPropagation);
    }
  }
  return bound;
}
...
var MatchDecorator = class {
  ...
  */
  createDeco(view) {
    ...
    for (let { from, to } of matchRanges(view, this.maxLength)) iterMatches(view.state.doc, this.regexp, from, to, (from2, m) => this.addMatch(m, view, from2, add));
    return build.finish();
  }
  /**
...
var Names = { 0: "null", 7: "bell", 8: "backspace", 10: "newline", 11: "vertical tab", 13: "carriage return", 27: "escape", 8203: "zero width space", 8204: "zero width non-joiner", 8205: "zero width joiner", 8206: "left-to-right mark", 8207: "right-to-left mark", 8232: "line separator", 8237: "left-to-right override", 8238: "right-to-left override", 8294: "left-to-right isolate", 8295: "right-to-left isolate", 8297: "pop directional isolate", 8233: "paragraph separator", 65279: "zero width no-break space", 65532: "object replacement" };
...
var lineNumberConfig = /* @__PURE__ */ Facet.define({ combine(values) {
  return combineConfig(values, { formatNumber: String, domEventHandlers: {} }, { domEventHandlers(a, b) {
...
var lineNumberGutter = /* @__PURE__ */ activeGutters.compute([lineNumberConfig], (state) => ({ class: "cm-lineNumbers", renderEmptyElements: false, markers(view) {
...
// /:https://framerusercontent.com/modules/CO9ruPlVDRTCLVInyri7/MGamyd2UebFXXyjBfB8y/lezer_common.js
    ...
    for (let parent = this; ; ) {
      for (let { children, positions } = parent._tree, e = dir > 0 ? children.length : -1; i2 != e; i2 += dir) {
        ...
        if (next instanceof TreeBuffer) {
          ...
          let mounted;
          if (!(mode & IterMode.IgnoreMounts) && (mounted = MountedTree.get(next)) && !mounted.overlay) return new _TreeNode(mounted.tree, start, i2, parent);
      ...
      if (this.stoppedAt != null && cursor.from >= this.stoppedAt) {
        ...
        if (overlay) {
          ...
          if (match) for (let r of match.mount.overlay) {
            ...
            if (from >= cursor.from && to <= cursor.to && !overlay.ranges.some((r2) => r2.from < to && r2.to > from)) overlay.ranges.push({ from, to });
          }
        }
        enter = false;
        ...
        if (range.from < range.to) {
          ...
          if (last >= 0 && overlay.ranges[last].to == range.from) overlay.ranges[last] = { from: overlay.ranges[last].from, to: range.to };
...
// /:https://framerusercontent.com/modules/rOWwbZHN39cczduPnzmw/YaHVqqT79imnSnw9WihN/lezer_highlight.js
var nextTagID = 0;
var Tag = class _Tag {
  /**
...
var Modifier = class _Modifier {
  static get(base2, mods) {
    ...
    for (let parent of base2.set) if (!parent.modified.length) for (let config of configs) set.push(_Modifier.get(parent, config));
    return tag;
  }
  constructor() {
    this.instances = [];
...
// /:https://framerusercontent.com/modules/aJsTBlWNkIaM900KNIXT/r60pzPp0bOIae0tDcSfS/codemirror_language.js
...
var top2 = typeof globalThis != "undefined" ? globalThis : typeof __unframerWindow2 != "undefined" ? __unframerWindow2 : {};
var StyleModule2 = class {
  ...
  static mount(root, modules, options) {
    ...
    if (!set) set = new StyleSet2(root, nonce);
    else if (nonce) set.setNonce(nonce);
    set.mount(Array.isArray(modules) ? modules : [modules], root);
  }
  // :: (Object<Style>, ?{finish: ?(string) → string})
...
var StyleSet2 = class {
  ...
  constructor(root, nonce) {
    ...
    if (!root.head && root.adoptedStyleSheets && win.CSSStyleSheet) {
      ...
      if (adopted) return root[SET2] = adopted;
      this.sheet = new win.CSSStyleSheet();
...
var LRLanguage = class _LRLanguage extends Language {
  ...
  */
  static define(spec) {
    ...
    return new _LRLanguage(data, spec.parser.configure({ props: [languageDataProp.add((type) => type.isTop ? data : void 0)] }), spec.name);
  }
  /**
  ...
  static getSkippingParser(until) {
    return new class extends Parser {
      createParse(input, fragments, ranges) {
        ...
        let parser = { parsedPos: from, advance() {
...
Language.state = /* @__PURE__ */ StateField.define({ create: LanguageState.init, update(value, tr) {
...
var language = /* @__PURE__ */ Facet.define({ combine(languages) {
  ...
  */
  lineAt(pos, bias = 1) {
    ...
    if (simulateBreak != null && simulateBreak >= line.from && simulateBreak <= line.to) {
      ...
      else if (bias < 0 ? simulateBreak < pos : simulateBreak <= pos) return { text: line.text.slice(simulateBreak - line.from), from: simulateBreak };
...
var HighlightStyle = class _HighlightStyle {
  ...
  */
  ...
  constructor(specs, options) {
    ...
    this.style = tagHighlighter(specs.map((style) => ({ tag: style.tag, class: style.class || def(Object.assign({}, style, { tag: null })) })), { all }).style;
...
var baseTheme2 = /* @__PURE__ */ EditorView.baseTheme({ "&.cm-focused .cm-matchingBracket": { backgroundColor: "#328c8252" }, "&.cm-focused .cm-nonmatchingBracket": { backgroundColor: "#bb555544" } });
var DefaultScanDist = 1e4;
var DefaultBrackets = "()[]{}";
var bracketMatchingConfig = /* @__PURE__ */ Facet.define({ combine(configs) {
  return combineConfig(configs, { afterCursor: true, brackets: DefaultBrackets, maxScanDistance: DefaultScanDist, renderMatch: defaultRenderMatch });
} });
...
var bracketMatchingState = /* @__PURE__ */ StateField.define({ create() {
  ...
  if (cursor && (dir < 0 ? cursor.childBefore(token.from) : cursor.childAfter(token.to))) do {
    if (dir < 0 ? cursor.to <= token.from : cursor.from >= token.to) {
      if (depth == 0 && matching.indexOf(cursor.type.name) > -1 && cursor.from < cursor.to) {
        ...
        if (depth == 0) {
          let endHandle = findHandle(cursor);
          return { start: firstToken, end: endHandle && endHandle.from < endHandle.to ? { from: endHandle.from, to: endHandle.to } : void 0, matched: false };
        }
        depth--;
      }
    }
...
function fullParser(spec) {
  return { name: spec.name || "", token: spec.token, blankLine: spec.blankLine || (() => {
  }), startState: spec.startState || (() => true), copyState: spec.copyState || defaultCopyState, indent: spec.indent || (() => null), languageData: spec.languageData || {}, tokenTable: spec.tokenTable || noTokens };
}
...
var Parse = class {
  ...
  finishChunk() {
    let tree = Tree.build({ buffer: this.chunk, start: this.chunkStart, length: this.parsedPos - this.chunkStart, nodeSet, topID: 0, maxBufferLength: 2048, reused: this.chunkReused });
...
function createTokenType(extra, tagStr) {
  ...
  if (!tag) return 0;
  let name2 = tagStr.replace(/ /g, "_"), type = NodeType.define({ id: typeArray.length, name: name2, props: [styleTags({ [name2]: tag })] });
  typeArray.push(type);
  return type.id;
}
function docID(data) {
...
// /:https://framerusercontent.com/modules/MiFWyNBz6FaRwOOEYJ4H/VH2xmlV2ZgVY4EG63Tp6/lezer_lr.js
...
var TokenCache = class {
  ...
  updateCachedToken(token, tokenizer, stack) {
    ...
    if (token.value > -1) {
      let { parser } = stack.p;
      for (let i2 = 0; i2 < parser.specialized.length; i2++) if (parser.specialized[i2] == token.value) {
...
var Parse2 = class {
  ...
  stackToTree(stack) {
    stack.close();
    return Tree.build({ buffer: StackBufferCursor.create(stack), nodeSet: this.parser.nodeSet, topID: this.topTerm, maxBufferLength: this.parser.bufferLength, reused: this.reused, start: this.ranges[0].from, length: stack.pos - this.ranges[0].from, minRepeatType: this.parser.minRepeatTerm });
  }
  stackID(stack) {
...
var LRParser = class _LRParser extends Parser {
  ...
  parseDialect(dialect) {
    ...
    let disabled = null;
    for (let i2 = 0; i2 < values.length; i2++) if (!flags[i2]) {
      for (let j = this.dialects[values[i2]], id2; (id2 = this.data[j++]) != 65535; ) (disabled || (disabled = new Uint8Array(this.maxTerm + 1)))[id2] = 1;
    }
  ...
  constructor(spec) {
    ...
    this.nodeSet = new NodeSet(nodeNames.map((name2, i2) => NodeType.define({ name: i2 >= this.minRepeatTerm ? void 0 : name2, id: i2, props: nodeProps[i2], top: topTerms.indexOf(i2) > -1, error: i2 == 0, skipped: spec.skippedNodes && spec.skippedNodes.indexOf(i2) > -1 })));
