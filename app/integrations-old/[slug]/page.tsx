...
export async function generateMetadata({
  ...
  return {
    title,
    description,
    alternates: { canonical: `/integrations-old/${slug}` },
    openGraph: {
      url: `https://velt.dev/integrations-old/${slug}`,
      title: `${title} | Velt`,
      description,
    },
  };
...
export default async function IntegrationPage({
  ...
  // Old detail pages now live under /integrations-old. Remap the shared card
  // hrefs (which point at /integrations/{slug}) to the moved namespace so the
  // "Explore Other Integrations" grid keeps working without touching shared
  // content or the new /integrations routes.
  const ownPath = `/integrations-old/${slug}`;
  const otherIntegrations = allIntegrationCards
    .map((card) => ({
      ...card,
      learnMoreHref: card.learnMoreHref?.replace(
        "/integrations/",
        "/integrations-old/",
      ),
    }))
    .filter((card) => card.learnMoreHref !== ownPath);
  ...
  const pageUrl = `${SITE_URL}/integrations-old/${slug}`;
  const breadcrumb = buildBreadcrumbList([
    { name: "Home", url: SITE_URL },
    { name: "Integrations", url: `${SITE_URL}/integrations-old` },
